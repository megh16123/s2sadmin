    ...
    if (
      ...
      return searchedData.map((item, index) => (
        <tr key={index}>
          ...
          <td>
            <RemoveModals email={item.email} data={item} page={props.page} />
            &nbsp;&nbsp;
            <MoreInfo data={item} page={props.page} />
            &nbsp;&nbsp;
            <EditInfoModals page={props.page} data={item} />
          </td>
        </tr>
      ));
    }
      ...
      return searchedData.map((item, index) => (
        <tr key={index}>
          ...
          <td>
            <RemoveModals email={item.email} data={item} page={props.page} />
            &nbsp;&nbsp;
            <MoreInfo data={item} amount={item.salary} page={props.page} />
            &nbsp;&nbsp;
            <EditInfoModals page={props.page} data={item} />
          </td>
        </tr>
    ...
    if (props.page === "home") {
      return data.map((item, index) => (
        <tr key={index}>
          ...
          <td>{item.message}</td>
          <td>
            <RemoveModals page={props.page} data={item} />
            &nbsp;&nbsp;
            <MoreInfo data={item} page={props.page} />
          </td>
        </tr>
    ...
    if (props.page === "enrollment") {
      return data.map((item, index) => (
        <tr key={index}>
          <td>{index + 1}</td>
          <td>{item.name}</td>
          <td>{item.classes.map((element) => element.class).join(", ")}</td>
          <td>
            <RemoveModals
              classesJoined={item.classes
    ...
    <div className="container">
      <div className="card mt-3">
        <div className="tableHeaderDiv">
          ...
          {(props.page === "student" || props.page === "teacher") && (
            <input
              className="search_input"
              type="text"
              placeholder="seacrh by name"
              value={searchQuery}
              onChange={(e) => searchEventHandler(e.target.value)}
            />
          )}
        </div>

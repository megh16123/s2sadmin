...
function RemoveModals(props) {
    ...
    const handleRemove = async () => {
        ...
        if (props.page === 'student') {
            try {
                const result = await axios.post('https://s2sapi.herokuapp.com/student/deletestudent', { email: props.data.email });
            ...
            try {
                const result = await axios.post('https://s2sapi.herokuapp.com/teacher/deleteteacher', { email: props.data.email });
                if (result.status === 200) {
                    handleShow2();
                    handleClose();
                }
        ...
        if (props.page === 'home') {
            try {
                const result = await axios.post('https://s2sapi.herokuapp.com/contactus/deletecontact', { id: props.data._id });
                if (result.status === 200) {
                    handleShow2();
                    handleClose();
                }
        ...
        if (props.page === 'enrollment') {
            try {
                const result = await axios.post('https://s2sapi.herokuapp.com/student/deleteenrolled', { email: props.data.email });
                if (result.status === 200) {
                    handleShow2();
                    handleClose();
                }
        ...
        <>
            ...
            >
                <Modal.Header closeButton>
                    {(props.page === 'teacher' || props.page === 'student') && <Modal.Title>Remove a {props.page}</Modal.Title>}
                    {props.page === 'enrollment' && <Modal.Title>Remove a registeration</Modal.Title>}
                ...
                <Modal.Body>
                    ...
                    <br />
                    {props.page === "student" && `Fee Due : ${totalfee - student.feepaid}`}
                    {props.page === "home" && `Message : ${message.message}`}
                    {props.page === "enrollment" && `Classes : ${props.classesJoined}`}
                    <br />
                    <br />
                    {(props.page === "teacher" || props.page === "student") && <span className="caution-message">{`NOTE: Once you remove the ${props.page}, all the information of the ${props.page}
                ...
                <Modal.Footer>
                    ...
                    {props.page === "enrollment" && `Do you want to remove this registeration?`}
            ...
            >
                <Modal.Header>
                    ...
                    {(props.page === "teacher" || props.page === "student") && <Modal.Title>{`${props.page} Removed`}</Modal.Title>}
                    {props.page === "home" && <Modal.Title>{`Message Removed`}</Modal.Title>}
                    {props.page === "enrollment" && <Modal.Title>{`Registeration Removed`}</Modal.Title>}
                </Modal.Header>
                <Modal.Body>

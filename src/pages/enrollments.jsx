...
function Enrollment() {
  ...
  useEffect(() => {
      ...
      const fetchData = async () => {
        try {
          const result = await axios(
            "https://s2sapi.herokuapp.com/student/getenrolled"
          );
          setData(result.data);
        } catch (error) {
          setData([]);
    ...
    return (
      <>
        <Header headerTitle="New enrollments" breadCrumb={['Dashboard','Enrollment']} />
        <MainSection page="enrollment" data={data} tableHeading="New Enrollments" column="Classes"/>
        <Footer />
      </>
    );
  }
